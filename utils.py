...
class record:
    ...
    def __init__(self):
        ...
        self.args = shlex.split('/usr/bin/arecord -Dac108 -f S32_LE -r 48000 -c 4 | /usr/bin/ffmpeg -re -i - -acodec pcm_s24be -f rtp rtp://10.94.0.31:23000 -sdp_file /home/pi/24.sdp')
    
    def record(self):
        if self.record_thread is not None and self.record_thread.poll() is None:
            return 'it\'s already recording'
        self.record_thread = subprocess.Popen(self.args, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        time.sleep(2)
